...
RSpec.describe Services::Notification do
  ...
  it "do not send question notification to answer author" do
    expect(NotificationMailer).not_to receive(:notify).with(answer_author, answer).and_call_original
  end

  it "do not send question notification to unsubscribers" do
    unsubscribers.each do |user|
      expect(NotificationMailer).not_to receive(:notify).with(user, answer).and_call_original
    end
  end
end
